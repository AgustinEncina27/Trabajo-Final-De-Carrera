...
public class ColoniaHormigas {
    ...
    private double calcularUtilidad(PuntoDeInteres destino) {
        ...
        // **Ajuste por preferencia de popularidad**
        int preferenciaPopularidad = perfilUsuario.getPopularidad(); // Valor entre 1 y 10
        int popularidadDestino = destino.getPopularidad(); // Valor entre 1 y 10

        // Diferencia entre la popularidad del destino y la preferencia del usuario
        int diferencia = Math.abs(preferenciaPopularidad - popularidadDestino);

        // Cuanto menor sea la diferencia, más se ajusta a la preferencia del usuario
        double ajustePopularidad = 1.0 - (diferencia / 10.0); // Normalizamos el ajuste entre 0 y 1

        // Aplicamos el ajuste a la utilidad total
        utilidadTotal *= (0.5 + ajustePopularidad); // Mínimo 50% si la diferencia es grande, hasta 100% si coinciden
